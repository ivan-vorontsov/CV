let assets = {
	...
	jsonExtensions: ["json"],
	audioExtension: ["mp3", "ogg", "wav", "webm"],
	...
	load(sources) {
		...
		return new Promise(resolve => {
			...
			sources.forEach(source => {
				...
				if(this.imageFileExtensions.indexOf(extension) !== -1) {
				...
				else if (this.audioExtension.indexOf(extension) !== -1) {
	...
	loadJson(source, loadHandler) {
		...
		xhr.onload = event => {
			if (xhr.status === 200) {
				...
				if (file.frames) {
					this.createTilesetFrame(file, source, loadHandler);
				} else {
					loadHandler();
				}
			}
	...
	createTilesetFrame(file, source, loadHandler) {
	...
	loadSound(source, loadHandler) {
		console.log("load sound called - details will come soon");
	}
};